...
import type { ApplicationWithChecked } from "@/types/roleEditTypes";

interface ApplicationPermissionSelectorWithCheckedProps {
  // Data aplikasi dengan status is_checked dari backend
  initialApplications: ApplicationWithChecked[];
...
const ApplicationPermissionSelectorWithChecked: React.FC<
  ApplicationPermissionSelectorWithCheckedProps
> = ({
  initialApplications,
  ...
  // Local state untuk applications dengan status is_checked yang bisa diupdate
  const [applications, setApplications] = useState<ApplicationWithChecked[]>(
    []
  );

  // Initialize applications dari prop dengan is_checked status
  useEffect(() => {
    setApplications(initialApplications);
  }, [initialApplications]);

  // Sync local state dengan perubahan dari parent component
  useEffect(() => {
    setApplications((prevApps) =>
      prevApps.map((app) => ({
        ...app,
        is_checked: selectedApplications.includes(app.id),
        permissions: app.permissions.map((perm) => ({
          ...perm,
          is_checked: selectedPermissions.includes(perm.id),
        })),
  ...
  }, [selectedApplications, selectedPermissions]);
  ...
  const getAppIcon = (iconName: string) => {
    const icons = {
      ...
      calendar: <BarChart3 className="h-5 w-5" />,
    };
    return (
  ...
  const handleApplicationToggle = (appId: number) => {
    ...
    const isCurrentlySelected = app.is_checked;
    let newSelectedApps: number[];
    let newSelectedPermissions: number[] = [...selectedPermissions];

    if (isCurrentlySelected) {
      // Remove application
      newSelectedApps = selectedApplications.filter((id) => id !== appId);

      // Remove all permissions from this app
    ...
    if (
      !isCurrentlySelected ||
      newSelectedPermissions.length !== selectedPermissions.length
    ) {
  ...
  const handleSelectAllMenuPermissions = (menu: any) => {
    const menuPermissionIds = menu.permissions.map((p: any) => p.id);
    const allSelected = menuPermissionIds.every((id: number) =>
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filteredApplications.map((application) => {
          const isAppSelected = application.is_checked; // Menggunakan is_checked dari state
                  ...
                  {application.menus.map((menu) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        >
                          {menu.permissions.every((p) => p.is_checked)
                            ? "Deselect All"
                            : "Select All"}
                        </Button>
                      </div>
...
export default ApplicationPermissionSelectorWithChecked;
