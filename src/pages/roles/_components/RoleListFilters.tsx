...
import { Search, Filter, X, Shield, Calendar, Users } from "lucide-react";
import type { RoleFilters } from "@/types/role";

interface RoleListFiltersProps {
  filters: RoleFilters;
  onFiltersChange: (filters: RoleFilters) => void;
  loading?: boolean;
}

const RoleListFilters: React.FC<RoleListFiltersProps> = ({
  filters,
  onFiltersChange,
  loading = false,
}) => {
  ...
  const handleAdminFilterChange = (isAdmin: string) => {
    const adminValue = isAdmin === "" ? "" : isAdmin === "true";
    onFiltersChange({
      ...filters,
      is_admin: adminValue as boolean | "",
      page: 1,
    });
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3 items-center bg-white p-3 rounded-lg border">
        ...
        <select
          className="border border-gray-300 rounded-md px-3 py-1.5 text-sm h-9 min-w-[120px]"
          value={
            filters.is_admin === "" ? "" : filters.is_admin ? "true" : "false"
          }
          onChange={(e) => handleAdminFilterChange(e.target.value)}
          disabled={loading}
        >
          <option value="">All Types</option>
          <option value="true">Admin Roles</option>
          <option value="false">User Roles</option>
        </select>
        ...
        <div className="bg-gray-50 p-4 rounded-lg border space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="permissions_count">Permission Count</option>
              </select>
            </div>
          ...
          <div className="pt-3 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant={filters.is_admin === true ? "default" : "outline"}
                size="sm"
                onClick={() =>
                  handleAdminFilterChange(
                    filters.is_admin === true ? "" : "true"
                  )
                }
              >
                Admin Roles
              ...
              <Button
                variant={
                  filters.sort_by === "users_count" &&
                  filters.sort_order === "desc"
                    ? "default"
                    : "outline"
                }
                size="sm"
                onClick={() => handleSortChange("users_count", "desc")}
              >
                Most Users
              </Button>
              <Button
        ...
        <div className="flex items-center gap-2 text-xs text-gray-600">
          ...
          {filters.is_admin !== "" && (
            <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded">
              Type: {filters.is_admin ? "Admin" : "User"} roles
            </span>
          )}
          {(filters.sort_by !== "created_at" ||
            filters.sort_order !== "desc") && (
